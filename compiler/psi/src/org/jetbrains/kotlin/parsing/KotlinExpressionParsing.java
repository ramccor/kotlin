...
public class KotlinExpressionParsing extends AbstractKotlinParsing {
    ...
    private static final BinaryOperationPrecedence MIN_BINARY_OPERATOR_PRECEDENCE = BinaryOperationPrecedence.AS;

    public static final BinaryOperationPrecedence MAX_BINARY_OPERATOR_PRECEDENCE = BinaryOperationPrecedence.ASSIGNMENT;

    public static Map<KtToken, BinaryOperationPrecedence> BINARY_OPERATOR_TO_PRECEDENCE;

    static {
        List<Map<KtToken, BinaryOperationPrecedence>> allBinaryOperations =
                getPrecedencesToOperations(MIN_BINARY_OPERATOR_PRECEDENCE,  /* considerSoftKeywordAsIdentifier = */ false);
        // Soft keywords work as identifiers (for INFIX functions).
        // However, they are being remapped to IDENTIFIER during parsing.
        // That's why there are no soft keywords as operators in the output tree, and the public `BINARY_OPERATOR_TO_PRECEDENCE` should not contain them.
        BINARY_OPERATOR_TO_PRECEDENCE = allBinaryOperations.get(allBinaryOperations.size() - 1);
    }

    private static final List<Map<KtToken, BinaryOperationPrecedence>> BINARY_PRECEDENCES_TO_CURRENT_AND_HIGHER_OPERATIONS =
            getPrecedencesToOperations(MIN_BINARY_OPERATOR_PRECEDENCE, /* considerSoftKeywordAsIdentifier = */ true);
    ...
    private static final List<Map<KtToken, BinaryOperationPrecedence>> BINARY_PRECEDENCES_TO_CURRENT_AND_HIGHER_OPERATIONS_UP_TO_IS =
            getPrecedencesToOperations(BinaryOperationPrecedence.IN_OR_IS, /* considerSoftKeywordAsIdentifier = */ true);

    private static List<Map<KtToken, BinaryOperationPrecedence>> getPrecedencesToOperations(
            BinaryOperationPrecedence startPrecedence,
            Boolean considerSoftKeywordAsIdentifier
    ) {
        ...
        for (BinaryOperationPrecedence entry : BinaryOperationPrecedence.values()) {
            if (entry.ordinal() < startPrecedence.ordinal()) {
                ...
                for (IElementType type : entry.operations.getTypes()) {
                    KtToken operation = (KtToken) type;
                    BinaryOperationPrecedence existingHigherOperation = currentOrHigherOperations.get(operation);

                    if (existingHigherOperation != null) {
                        throw new IllegalArgumentException(
                                "All precedences have unique operations. The " + operation +
                                " already assigned to " + existingHigherOperation + "."
                        );
                    }

                    currentOrHigherOperations.put(operation, entry);

                    if (type == IDENTIFIER && considerSoftKeywordAsIdentifier) {
                ...
                result.add(currentOrHigherOperations);
            }
        }
    ...
    static {
        ...
        allOperations.addAll(BINARY_OPERATOR_TO_PRECEDENCE.keySet());
    ...
    public void parseExpression() {
        ...
        parseBinaryExpression(MAX_BINARY_OPERATOR_PRECEDENCE);
    }
