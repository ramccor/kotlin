...
class WasmBaseTypeOperatorTransformer(val context: WasmBackendContext) : IrElementTransformerVoidWithContext() {
    ...
    private fun possiblyGenericTypeCast(expression: IrTypeOperatorCall) =
        expression.argument.type.classOrNull?.owner?.isEffectivelyExternal() != true &&
                expression.possibleGenericTypeUpCast == true &&
    ...
    private fun lowerImplicitCast(expression: IrTypeOperatorCall): IrExpression =
        if (possiblyGenericTypeCast(expression)) {
