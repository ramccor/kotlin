...
class GenConsistencyTest {
    ...
    fun testGeneratedApiClassesAreUpToDate() {
        ...
        checkPathHash(apiGenPath, -1741780087, "kotlin-build-tools-api")
    }
    ...
    fun testGeneratedImplClassesAreUpToDate() {
        ...
        checkPathHash(apiGenPath, -1022948609, "kotlin-build-tools-impl")
    }
}
...
private fun checkPathHash(apiGenPath: File, expectedHash: Long, moduleName: String) {
    ...
    for (genFile in apiGenFiles) {
        hash += genFile.relativeTo(apiGenPath).path.hashCode() + genFile.readText().hashCode()
    }
    assertEquals(
        expectedHash,
        hash,
